...
class PegSolitaire{
    ...
    public:
        ...
        PegSolitaire(const int& typOfbrd){//constructor
            boardType = typOfbrd;
            gameName = "Type ";
        ...
        PegSolitaire(const int& typOfbrd,const string& name){//constructor
            boardType = typOfbrd;
            gameName = name;
            createBoard(boardType);
        }

        void getname() const{ //get func
            cout << gameName;
        }
        
        void printBoard(); //prints gameboard
        int numberOfPegs(); //returns number of pegs
        int numberOfEmpty();  // returns number of empty cells
        inline int numberOfTakenPeg() {return numberOfEmpty() -1;} // returns number of taken pegs

        void play(); // plays automatically one step
        void play(const int& rw,const char& clmn,const char& direction);// plays one step with input
        bool finish_test(); // checks if game is over

        void playGame(); // plays automatically until game is over 

        void boardSave(const string& filename); // saves gameboard to file
        void boardLoad(const string& filename); // loads gameboard from file

        bool compareGames(PegSolitaire secondGame); 

        static int numOfTotalPeg(){ // returns total number of pegs in all games
            return totalPeg;
        }

        void delTotalPeg(){ // if one of the boards is deleted, it subtracts the number of pegs in the deleted game from the total number of pegs
