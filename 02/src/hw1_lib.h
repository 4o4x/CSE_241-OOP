...
void player_human(vector < vector<board> > &map,int numberOfMoves = 0); // It takes command from user and plays on board

void player_cpu(vector < vector<board> > &map,int numberOfMoves = 0); // It plays randomly on the board until there are no pegs to move legally
...
void boardSave(const string& filename,const vector < vector<board> > & map,int playerType,int numberOfMoves); // saves the current game status to .txt file
